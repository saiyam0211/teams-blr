...
const TeamPage = () => {
  ...
  return (
    <div className="text-white min-h-screen flex flex-col items-center bg-black bg-cover bg-[image:url(./assets/bg-bad.png)]">
      {/* Header */}
      <div className="relative w-full max-w-7xl text-center pt-6 px-4">
        <h1 className="font-bold mx-auto -mt-20 sm:ml-96 md:ml-96 lg:ml-96 xl:ml-96">
          <img
            src={Logo}
            alt="logo"
            className="mt-6 mx-auto sm:ml-96 md:ml-64 lg:ml-64 xl:ml-64" // Logo positioning for larger screens
            height={100}
            width={300}
          />
        </h1>
        <p className="text-lg -mt-20 sm:text-center sm:px-4 lg:ml-72">Powered by Mittal Alliance</p>
        <h2 className="text-4xl sm:text-5xl sm:text-center sm:px-4 mt-4 anton font-black lg:text-6xl xl:text-6xl lg:ml-60">
          BENGALURU TEAM
        </h2>
      </div>

      {/* Team Section */}
      <div className="relative flex justify-center items-center w-full max-w-6xl mt-8 lg:mt-0">
        <div className="relative w-full h-[400px]">
          {teamMembers.map((member, index) => (
            <div
              key={index}
